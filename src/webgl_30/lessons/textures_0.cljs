...
(let [img (js/Image.)]
  (aset img "src" "images/f-texture.png")
  (.addEventListener img
                     "load"
                     (fn []
                       (let [{:keys [gl]} @state-atom]
                         (webgl/create-texture-with-mipmap gl
                                                           [(.-TEXTURE_2D gl)
                                                            0
                                                            (.-RGBA gl)
                                                            (.-RGBA gl)
...
(defn setup!
  ...
  (-> (swap! state-atom (fn [{:keys [gl rect] :as state}]
                          (let [
                            ...
                            (webgl/create-texture! gl f-texture)
  ...
  {:title           (fn []
                      [:div
                       ...
                       [:h4 {:style {:font-family "monospace"}}
                        "Texture"]])
   :source          "https://github.com/erichall/webgl-30/blob/master/src/webgl_30/lessons/textures_0.cljs"
   :tutorial-source "https://webglfundamentals.org/webgl/lessons/webgl-3d-textures.html"
   :start           (fn []
